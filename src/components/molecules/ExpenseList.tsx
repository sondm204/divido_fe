...
export const ExpenseList = (props: ExpenseProps) => {
  ...
  const expenseList = useSelector(
    (state: RootState) =>
      state.groupEditor.groups?.find((g) => g.id === selectedGroupId)?.expenses
  );
  ...
  const handleEditExpense = (expenseId: string) => {
    ...
    const editExpense = expenseList?.find(
          ...
          {Array.isArray(expenseList) &&
            expenseList.map((item: Expense, index) => (
              <tr key={index}>
                ...
                <td className="px-4 py-2">
                  {item.id === editExpenseId ? (
                    <Input iSize={"small"} type="text" defaultValue={editNote} />
                  ) : (
                    item.note
                  )}
                </td>
                ...
                <td className="flex gap-2 px-4 py-2">
                  <div className="flex items-center gap-2">
                    {/* <CustomDialog
                      ...
                      dialogTitle="Chi tiết hóa đơn ngày (đang fix cứng)"
                      content={<BillList selectedExpenseId={item.id} />}
                    /> */}
                    {item.id === editExpenseId ? (
                      <Save
                        size={20}
